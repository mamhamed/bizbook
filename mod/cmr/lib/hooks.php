...
function cmr_user_hover_menu($hook, $type, $return, $params) {
	...
	if (!empty($params) && is_array($params) && ($user = elgg_get_logged_in_user_entity())) {
		...
		if (elgg_instanceof($user, "user", "client_subtype") && elgg_instanceof($target_user, "user", "client_subtype") && $user->getGUID() != $target_user->guid) {
			...
			// If c2b relationship exists, include 'Revoke as a service provider' option in the menu. Else, include 'Select as a service provider' option in the menu.
			if (check_entity_relationship($user->getGUID(), 'c2b', $target_user->guid)) {
				$url = elgg_add_action_tokens_to_url("action/c2b_revoke?object_guid=$target_user->guid");
				$item = new ElggMenuItem('c2b_revoke', elgg_echo('cmr:c2b_revoke:user'), $url);
			} else {
				$url = elgg_add_action_tokens_to_url("action/c2b_establish?object_guid=$target_user->guid");
				$item = new ElggMenuItem('c2b_establish', elgg_echo('cmr:c2b_establish:user'), $url);
			}
			$item->setSection('action');
			$return[] = $item;
