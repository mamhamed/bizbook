<?php
	...
	if(empty($user) || empty($friend)) {
		$errors = true;
		register_error(elgg_echo("cmr:friend_request:add:failure"));
	} else {
		...
		if(check_entity_relationship($friend->getGUID(), "friend", $user->getGUID())) {
			try {
				...
				system_message(elgg_echo("cmr:friends:add:successful", array($friend->name)));
				...
				register_error(elgg_echo("cmr:friends:add:failure", array($friend->name)));
				$errors = true;
			}
			...
			try {
				...
				if($result == false) {
					$errors = true;
					register_error(elgg_echo("cmr:friend_request:add:exists", array($friend->name)));
				}
	...
	if(!$errors) {
		system_message(elgg_echo("cmr:friend_request:add:successful", array($friend->name)));
	}
