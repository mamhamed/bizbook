<?php
	...
	if ($friend = get_user($friend_guid)) {
		...
		try{
			...
			register_error(elgg_echo("cmr:friends:remove:failure", array($friend->name)));
			$errors = true;
		}
	} else {
		register_error(elgg_echo("cmr:friends:remove:failure", array($friend_guid)));
		$errors = true;
	}
	
	if(!$errors) {
		system_message(elgg_echo("cmr:friends:remove:successful", array($friend->name)));
	}			
